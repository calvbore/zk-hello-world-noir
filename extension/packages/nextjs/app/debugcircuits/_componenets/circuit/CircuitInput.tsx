...
const checkError = (
  ...
  switch (abiType.kind) {
    ...
    case "tuple":
      ...
      if (
        obj.length != abiType.fields.length
      ) {
        return true;
      }
      return false;
      ...
      return [(
        <>
          <div className={`flex items-center ml-2 mb-2`}>
            <span className={`ml-${Math.min(path.length-1, 4)} flex items-center block text-xs font-extralight leading-none`}>
              {typeTitle}
            </span>
          </div>
          <div className="mb-2">
        ...
        <>
          <div className="flex items-center ml-2 mb-2">
            <span className={`ml-${Math.min(path.length-1, 4)} flex items-center block text-xs font-extralight leading-none`}>
              ...
              >
                {inputIndividualMode ? (
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-3 ml-1 mr-1">
      ...
      case "struct":
        ...
        for (let i = 0; i<abiType.fields.length; i++) {
          ...
          fields.push(
            <>
              <div className="flex items-center ml-2 mb-2">
                <span className={`ml-${Math.min(path.length-1, 4)} flex items-center block text-xs font-medium leading-none`}>
    ...
    <>
      <div className="flex flex-col gap-1.5 w-full">
        ...
        <div className="mb-2 mt-2">
          {renderInput(
            inputParam.type, 
            [structuredClone(inputParam.name)], 
            stateMutateFunc, 
            inputs, 
            surfaceError,
          )[0]}
